...
// Nav items — role filtering happens at render time
const allNavItems = [
  { label: 'Dashboard', title: 'Dashboard', to: '/dashboard',          icon: DashboardIcon, allow: ['admin', 'editor'] },
  { label: 'Articles',  title: 'Articles',  to: '/dashboard/articles', icon: ArticleIcon,   allow: ['admin', 'editor'] },
  { label: 'Reports',   title: 'Reports',   to: '/dashboard/reports',  icon: AssessmentIcon,allow: ['admin', 'editor'] },
  { label: 'Users',     title: 'Users',     to: '/dashboard/users',    icon: PeopleIcon,    allow: ['admin'] },
];
...
const DashLayoutInner = () => {
  ...
  const visibleNavItems = allNavItems.filter((item) => item.allow.includes(role));

  const getPageTitle = (pathname) =>
    allNavItems.find(({ to }) => to === pathname)?.title ?? 'Welcome';
      ...
      <AppBar position="fixed" open={open} elevation={0} sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
        <Toolbar>
          ...
          <Typography variant="h6" noWrap component="div" sx={{ flexGrow: 1, fontWeight: 600 }}>
            {firstName ? `Welcome, ${firstName}` : getPageTitle(location.pathname)}
          </Typography>
            ...
            return (
              <ListItem key={to} disablePadding sx={{ display: 'block', mb: 0.5 }}>
                <Tooltip title={open ? '' : label} placement="right">
                  ...
                  >
                    <ListItemIcon sx={{ minWidth: 0, mr: open ? 2 : 'auto', justifyContent: 'center', color: isSelected ? 'white' : 'text.secondary' }}>
                      <Icon fontSize="small" />
                    </ListItemIcon>
                    <ListItemText
                      primary={label}
